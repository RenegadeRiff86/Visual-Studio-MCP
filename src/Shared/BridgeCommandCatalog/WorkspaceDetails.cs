    ...
    private static bool TryGetWorkspaceCommandDetail(string commandName, out (string Description, string Example) detail)
    {
        switch (commandName)
        {
            ...
            case "batch":
                ...
                return true;
            case "open-document":
                detail = ("Open a document by absolute path, solution-relative path, or solution item name.", ExampleCommand("open-document", @"{""file"":""src\\CMakeLists.txt"",""line"":1,""column"":1}"));
                return true;
            case "list-documents":
            ...
            case "close-document":
                ...
                return true;
            case "save-document":
                detail = ("Save one document by path or save all open documents.", ExampleCommand("save-document", @"{""file"":""C:\\repo\\src\\foo.cpp""}"));
                return true;
            case "close-file":
                detail = ("Close one open file tab by path or query.", ExampleCommand("close-file", @"{""file"":""C:\\repo\\src\\foo.cpp""}"));
                return true;
            case "close-others":
            ...
            case "execute-command":
                ...
                return true;
            case "document-slice":
                detail = ("Fetch one code slice from a file.", ExampleCommand("document-slice", @"{""file"":""C:\\repo\\src\\foo.cpp"",""line"":120,""context_before"":8,""context_after"":20}"));
                return true;
            case "document-slices":
                detail = ("Fetch multiple code slices from ranges_file or inline ranges JSON.", ExampleCommand("document-slices", @"{""ranges"": [{""file"": ""C:\\repo\\src\\foo.cpp"", ""line"": 42, ""context_before"": 8, ""context_after"": 20}]}"));
                return true;
            case "apply-diff":
