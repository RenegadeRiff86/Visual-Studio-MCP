    ...
    private static bool TryGetProjectCommandDetail(string commandName, out (string Description, string Example) detail)
    {
        switch (commandName)
        {
            ...
            case "query-project-properties":
                detail = (
                    "Read MSBuild-style project properties from one project, including normalized TargetFramework values when available.",
                    ExampleCommand("query-project-properties", @"{""project"":""VsIdeBridge"",""names"": [""TargetFramework"", ""RootNamespace"", ""AssemblyName""]}"));
                return true;
            ...
            case "query-project-references":
                detail = (
                    "List project references for one project. By default this returns resolved references with framework assemblies omitted; set declared_only=true for project-file declarations or include_framework=true for the full closure.",
                    ExampleCommand("query-project-references", @"{""project"":""VsIdeBridge.Tests"",""declared_only"":true}"));
                return true;
            case "query-project-outputs":
                detail = (
                    ...
                    ExampleCommand("query-project-outputs", @"{""project"":""VsIdeBridge"",""configuration"":""Release"",""target_framework"":""net8.0""}"));
                return true;
            case "add-project":
            ...
            case "rename-project":
                detail = (
                    "Rename a project within the solution. This changes the project name shown by Visual Studio, but does not rename folders or the project file on disk.",
                    ExampleCommand("rename-project", @"{""project"":""MyLib"",""new-name"":""MyLibrary""}"));
                return true;
            ...
            case "search-solutions":
                detail = (
                    ...
                    ExampleCommand("search-solutions", @"{""query"":""MyApp"",""path"":""%USERPROFILE%\\source\\repos"",""max_depth"":4}"));
                return true;
            case "set-python-project-env":
                detail = (
                    "Set the active Python interpreter for the open .pyproj project or open-folder workspace in Visual Studio (affects IntelliSense and debugging).",
                    ExampleCommand("set-python-project-env", @"{""path"":""%USERPROFILE%\\miniconda3\\envs\\superslicer\\python.exe""}"));
                return true;
