...
- Fixed `git_add`, `git_reset`, and `git_restore` using LibGit2Sharp in-process instead of a subprocess. The subprocess hung for 30 seconds under the Windows service account due to index lock contention with VS's own git integration; LibGit2Sharp avoids all subprocess overhead and works reliably in the service context.
